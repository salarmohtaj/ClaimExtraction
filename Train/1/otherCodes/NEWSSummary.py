...
content = Field(tokenize="spacy", lower=True, init_token="<sos>", eos_token="<eos>")
#claim = Field(tokenize="spacy", lower=True, init_token="<sos>", eos_token="<eos>")
fields = [(None, None), (None, None), ('claim',content), (None, None),('content', content)]

training_data=data.TabularDataset(path = 'data/news_summary1.csv',format = 'csv',fields = fields,skip_header = True)
train_data, test_data, valid_data = training_data.split(split_ratio=[0.4, 0.1, 0.5])
content.build_vocab(train_data, max_size=10000, min_freq=3, vectors = "glove.6B.100d")
#claim.build_vocab(train_data, max_size=10000, min_freq=3, vectors = "glove.6B.100d")
...
input_size_decoder = len(content.vocab)
...
output_size = len(content.vocab)

decoder_lstm = DecoderLSTM(input_size_decoder, decoder_embedding_size,
                           hidden_size, num_layers, decoder_dropout, output_size).to(device)
print(decoder_lstm)
# Initialize the pretrained embedding
pretrained_claim_embeddings = content.vocab.vectors
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source, target, tfr=0.5):
        ...
        target_vocab_size = len(content.vocab)
...
pad_idx = content.vocab.stoi["<pad>"]
...
def translate_sentence(model, sentence, content, device, max_length=50):
    ...
    outputs = [content.vocab.stoi["<sos>"]]
    ...
    for _ in range(max_length):
        ...
        if output.argmax(1).item() == content.vocab.stoi["<eos>"]:
            break

    translated_sentence = [content.vocab.itos[idx] for idx in outputs]
    return translated_sentence[1:]

def bleu(data, model, content, device):
    targets = []
    outputs = []
    ...
    for example in data:
        ...
        prediction = translate_sentence(model, src, content, device)
...
for epoch in range(num_epochs):
    ...
    print()
    score = bleu(test_data[1:5], model, content, device)
